...
| Done | Cognitive Overload — `ArtBackend` is a symbol registry, capability checker, and feature executor at once | `ArtBackend` now keeps construction and shared helpers central while method replacement, deoptimization, enumeration, method query, and heap enumeration behavior live beside their feature modules. |
| Done | Knowledge Duplication — JNI primitive slot knowledge is repeated in several forms | Consolidated primitive call, field, and original-call JNI slot/function metadata into one local macro audit table in `src/env/macros.rs`, with host tests asserting the expected vtable slots. |
...
| Partial | Dependency Disorder — Parent-module preludes hide real dependency flow | Touched Java wrapper modules now use explicit imports after the wrapper split; broader java/env prelude cleanup remains. |
| Partial | Accidental Complexity — Replacement path is split more finely than the current backend needs | Removed the one-use `src/java/replacement/backend.rs` wrapper; closure replacement now owns the ART guard directly, keeps restore/leak lifecycle behavior local, resolves method/constructor targets through one `MethodKind` match, and ART replacement install/restore/prerequisite logic now lives in `src/art/replacement.rs`. Original-call lookup, bypass, and JNI dispatch now live with `OriginalMethod`, and the one-use `src/java/replacement/original_call.rs` module was removed. More simplification remains because the Java replacement flow still spans many modules. |
...
| Done | Cognitive Overload — The Java facade’s data model and behavior are split awkwardly | Moved Java wrapper data types and behavior out of `src/java/mod.rs`/`src/java/wrapper.rs` into class, object, array, members, and returns modules; removed `src/java/wrapper.rs` while keeping public re-exports stable. |
| Done | Accidental Complexity — `ArtBackend` is a central symbol bag for many independent ART features | Grouped resolved ART symbols into common, enumeration, heap, and deoptimization structs; `ArtBackend` remains the single facade while feature modules read only the symbol groups they need. |

Progress counts: Done 9 / Partial 2 / Open 0 / Total 11.
