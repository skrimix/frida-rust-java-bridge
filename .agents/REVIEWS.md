...
| Done | Accidental Complexity — Replacement path is split more finely than the current backend needs | Removed the one-use Java replacement backend and install wrappers; closure replacement owns the ART guard directly, while public replace impls own facade admission and callback wrapping. Original calls now dispatch from the active invocation metadata instead of a duplicate `OriginalMethod` state object. Low-level trampoline, return, argument, and constructor-safety modules remain separate because each owns distinct behavior. |
...
Progress counts: Done 10 / Partial 1 / Open 0 / Total 11.
