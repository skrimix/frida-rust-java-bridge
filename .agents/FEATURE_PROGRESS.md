...
| Main-thread scheduling | Partial | `Java::is_main_thread()`, `Java::schedule_on_main_thread()`, `MainThreadTaskHandle::wait_for_completion()`, `JavaCapabilities::main_thread_scheduling` | Uses `Looper` checks, a process-global Rust queue, `Handler(Looper.getMainLooper()).sendEmptyMessage(1)`, and a Gum `epoll_wait` hook to drain queued callbacks on the main thread. Capability reporting checks scheduler prerequisites without enqueueing or waking the looper. Task handles can wait with a timeout for completion or failure. Command-line `app_process` reports unsupported when no main looper exists; APK early-start validation covers real main-looper drain. |
...
| Constructors, methods, static methods, fields | Done | `JavaClass::{new_object,new_object_with,constructor}`, `JavaConstructor`, `JavaMethod`, `JavaField` | Reflection-validated overload/member handles with typed helpers for primitive, object, and array fields; selected instance handles validate receiver class before JNI; exact selected calls validate reference argument assignability; `new_object()` and name-only calls use ranked argument dispatch, while exact overload handles remain available. |
| Automatic JS-style overload dispatch | Done | `JavaClass::call`, `JavaObject::call`, `JavaBoundObject::call`, `JavaClass::new_object` | Name-only calls and constructors pick the best compatible overload using a deterministic Frida-like ranking. This is not full Java compiler overload resolution; use exact overload selection when a broad compatible overload should be chosen deliberately. |
...
| Default app class loader | Done | `Java::perform()`, `Java::app_class_loader()`, `Java::wait_for_app_loader()`, `Java::default_app_loader()` | `perform()` is the normal app-class entry point. `wait_for_app_loader()` returns immediately when the loader is already known or `ActivityThread.currentApplication()` is currently available; otherwise it can block with a timeout on a background/helper thread by installing the same deferred startup hooks as `perform()`. Successful wait/perform paths publish the default app loader used by bare `use_class()` wrapper lookup; low-level `find_class()` remains bootstrap-scoped on bare handles. Deferred startup remains covered by the `Java.perform()` row. |
...
| Typed facade argument/return helpers | Done | `JavaHookContext::{args,arg_value,arg_display,arg_is_null,arg::<String>,this_object,arg_object,arg_array,call_original::<String>}`, `JavaHookArgument`, `JavaLocalObject`, `JavaLocalArray`, safe wrapper and string hook returns | Primitive/string conversions, diagnostic argument display, safe iterable argument views, callback-local reference views, typed array extraction through `call_original`, safe object/object-array wrapper returns, owned original object/array returns, invocation-local object/array return conversion, return assignability checks, and null-reference handling are covered through the public closure trampoline path. |
| Java-aware diagnostic display | Done | `java_to_string()` on Java objects and arrays; `java_display()` on returns and hook arguments; explicit metadata accessors | Provides console-log-style diagnostic text for values. Values use direct primitive/null/void formatting and Java `Object.toString()` for references; arrays intentionally keep Java array `toString()` output. Class/member wrappers expose metadata through accessors such as `name()`, `signature()`, and `ty()`; descriptor values expose `descriptor()` for JNI text. |
...
| Arbitrary signatures and multi-reference args | Done | guarded facade plus internal raw-closure coverage | Public `replace()` admits arbitrary non-constructor descriptors that fit the current closure replacement limits. Constructor replacement uses the same guarded callback shape and admits void constructor descriptors that fit those limits; broader raw JNI-native admission remains deferred. |
