...
impl Env<'_> {
    ...
    /// # Safety
    ///
    /// `raw` must be a valid method ID for a class in this process's ART runtime, and `kind`/`signature` must describe
    ...
    /// # Safety
    ///
    /// `raw` must be a valid field ID for a class in this process's ART runtime, and `kind`/`ty` must describe that
    /// field accurately.
    ...
    /// object for this process's ART runtime, and `kind`/`signature` must accurately describe that reflected member.
    ...
    /// # Safety
    ///
    /// `field` must be a valid `java.lang.reflect.Field` object for this process's ART runtime, and `kind`/`ty` must
    ...
    /// # Safety
    ///
    /// `method` must have been resolved from `class` in this process's ART runtime, and `class` must still name the
    ...
    /// # Safety
    ///
    /// `field` must have been resolved from `class` in this process's ART runtime, and `class` must still name the
    /// declaring class identity expected by that JNI ID.
