...
impl ArtBackend {
    ...
    pub(crate) fn enumerate_class_loaders(&self, vm: &Vm) -> Result<Vec<ClassLoaderRef>> {
        ...
        with_runnable_art_thread(&env, |thread| {
...
fn with_runnable_art_thread(
    ...
    // This is the current API 26+ arm64 v1 path. ART stores the current art::Thread* in
    // JNIEnvExt immediately after the JNI function table. A follow-up should replace this with
    // the full upstream ExceptionClear recompiler before broadening version support.
    f(thread)
}
