...
pub(super) unsafe extern "C" fn replacement_answer_calling_original(
    ...
    match unsafe { replacement::call_original_static_i32_method(env, class, "answer") } {
        Ok(value) => value + 1000,
        Err(error) => {
...
pub(super) unsafe extern "C" fn replacement_static_echo_calling_original(
    ...
    match unsafe {
        replacement::call_original_static_method(
            env,
            class,
            "staticEcho",
        ...
        Ok(replacement::RawJavaReturn::Object(value)) => {
            let expected_argument = EXPECTED_ARGUMENT.load(Ordering::SeqCst);
            if argument.is_null() {
                if value.is_null() {
                    REPLACEMENT_STRING.load(Ordering::SeqCst)
...
pub(super) unsafe extern "C" fn replacement_static_add_calling_original(
    ...
    match unsafe {
        replacement::call_original_static_method(env, class, "staticAdd", "(II)I", (left, right))
    }
...
pub(super) unsafe extern "C" fn replacement_instance_number_calling_original(
    ...
    match unsafe { replacement::call_original_instance_i32_method(env, receiver, "instanceNumber") }
    {
        Ok(value) => value + 100,
        Err(error) => {
...
unsafe fn replacement_startup_shape_calling_original<const N: usize>(
    ...
    match unsafe {
        replacement::call_original_instance_method(env, receiver, name, signature, args)
    }
