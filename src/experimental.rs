...
impl ImplementationReturn {
    ...
    fn from_raw_for_type(value: RawJavaReturn, return_type: &JavaType) -> Self {
    ...
    fn into_raw(self) -> RawJavaReturn {
        match self {
...
/// Replaces a selected overload with a Rust `.implementation`-style closure.
///
/// This is a friendlier wrapper over `replace_closure_method`. It keeps the same experimental ART
/// backend, signature support, failure recording, and guard lifecycle while exposing borrowed
/// object/array return helpers instead of requiring callers to construct `RawJavaReturn` directly.
///
/// # Safety
///
