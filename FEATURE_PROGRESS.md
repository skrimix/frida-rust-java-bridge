...
| Raw class/object access | Done | `java::raw::Class`, `JavaRef`, `JavaReturn` | Global-reference-backed helpers, including `new_object_ref()`, over low-level `Env`, kept out of the default facade vocabulary. |
| Rust-native `Java.use()`-style wrappers | Done | `Java::use_class()`, `JavaClass`, `JavaObject` | Default high-level facade with explicit overload selection and wrapper-bound object views. Bare handles prefer the published default app loader for wrapper lookup, while explicit loader handles preserve their scope. |
