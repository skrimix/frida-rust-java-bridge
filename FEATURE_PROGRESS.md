...
| Public guarded replacement facade | Done | `JavaMethodOverload::install_implementation`, `replacement::{ImplementationInvocation, ImplementationReturn, ImplementationGuard}` | Soft-frozen for the currently admitted and test-covered descriptor-driven lanes, including multi-reference, mixed primitive, wide primitive, float-mix, and stack-spill shapes. Unsupported signatures fail before installation with errors naming method kind, name, descriptor, and admitted lanes. |
...
| Arbitrary signatures and multi-reference args | Partial | guarded facade plus hidden raw-closure coverage | Public `install_implementation()` admits the descriptor-driven shapes covered by live app-process checks. Internal closure layout can map more non-constructor descriptors, but arbitrary public descriptor admission remains intentionally deferred. |
