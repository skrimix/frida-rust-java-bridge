...
| `.implementation`-style API | Experimental | `JavaMethodOverload::implementation`, `replacement::{ImplementationInvocation, ImplementationReturn, ImplementationGuard}` | Guarded Rust closure API over the existing closure backend. It keeps explicit guard/revert lifecycle by design, adds typed argument/original-call helpers and primitive return conversions, and supports only the currently admitted closure ABI subset. |
