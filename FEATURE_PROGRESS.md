...
| Overload replacement ergonomics | Experimental | `JavaMethodOverload::install_implementation` | Public replacement surface is intentionally pruned to the guarded overload-install path. Raw/native overload helpers remain crate-internal scaffolding. |
| `install_implementation` API | Experimental | `JavaMethodOverload::install_implementation`, `replacement::{ImplementationInvocation, ImplementationReturn, ImplementationGuard}` | Guarded Rust closure API over the existing closure backend. It keeps explicit guard/revert lifecycle by design, preserves full argument-list inspection for exploratory hooks, adds typed argument/original-call helpers and primitive return conversions, and supports only the currently admitted closure ABI subset. This is a soft-freeze candidate after more matrix runs. |
