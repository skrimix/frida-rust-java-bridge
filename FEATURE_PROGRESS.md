...
| Raw closure-backed replacement callbacks | Experimental | internal raw closure backend plus descriptor-driven arm64 layout foundation | Raw invocation closures are supported for selected overload ABI shapes as crate-internal scaffolding for the public facade, startup hooks, and live-runtime harnesses. Generic AAPCS64 argument/return layout classification is present, but the executable trampoline has not yet been generalized. |
| Arbitrary signatures and multi-reference args | Planned | descriptor-driven layout foundation | Current accepted ABI shapes are intentionally narrow until the generic trampoline captures register and stack-passed arguments into the closure dispatcher. |
