...
| Public guarded replacement facade | Done | `JavaMethod::replace`, `JavaConstructor::replace`, `replacement::{JavaHookContext, JavaConstructorHookContext, JavaConstructorInitialized, JavaHookArgument, JavaHookReturn, JavaHookGuard}` | `JavaHookReturn` is the raw-reference `JavaValue` specialization used at the method hook boundary; primitive, null, owned wrapper, borrowed wrapper, and callback-local object/array returns are safe through `IntoJavaHookReturn`, with invocation-local views converted through `JavaHookContext::return_value()` / `IntoJavaHookReturn::into_hook_return(&ctx)` because direct `Ok(JavaLocalObject<'_>)` cannot fit `replace()`'s single return type. Raw JNI returns remain explicit `unsafe`. `JavaMethod::replace` is safe; `JavaConstructor::replace` is safe through an initialization token returned by original-constructor calls, while unchecked constructor replacement remains explicit `unsafe`. Supports arbitrary non-constructor descriptors handled by the descriptor-driven arm64 closure path, with live coverage for multi-reference, mixed primitive, wide primitive, float-mix, array, stack-spill, safe argument iteration, callback-local object/array returns, and invalid object-return rejection. Unsupported signatures fail before installation with errors naming method kind, name, and a concise reason. |
