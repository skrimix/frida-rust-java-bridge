...
| Arbitrary signatures and multi-reference args | Planned | hidden raw-closure coverage | Internal closure coverage now includes multi-reference, mixed primitive, wide primitive, and stack-spill shapes. Public `install_implementation()` admission remains intentionally narrow until broader live-device coverage agrees with the direct and raw JNI-native paths. |
