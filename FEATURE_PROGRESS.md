...
| Heap instance enumeration | Partial | `Java::choose_instances()`, `JavaClass::choose_instances()`, `JavaCapabilities::heap_enumeration` | Exact-class ART heap enumeration using a callback that returns `JavaChooseControl::{Continue, Stop}`. Only Android <12 supported. Supported layouts report experimental; missing ART heap symbols/layouts return structured unsupported errors. |
...
| Public guarded replacement facade | Done | `JavaMethod::replace`, `replacement::{JavaHookContext, JavaHookReturn, JavaHookGuard}` | Soft-frozen for arbitrary non-constructor descriptors handled by the descriptor-driven arm64 closure path, with live coverage for multi-reference, mixed primitive, wide primitive, float-mix, array, and stack-spill shapes. Unsupported signatures fail before installation with errors naming method kind, name, and a concise reason. |
| Calling original implementation from facade callbacks | Done | `JavaHookContext::{call_original,call_original_as}` | Covered for tested static and instance lanes. Thread-scoped raw original handles remain internal. |
...
| Arbitrary signatures and multi-reference args | Done | guarded facade plus hidden raw-closure coverage | Public `replace()` admits arbitrary non-constructor descriptors that fit the current closure replacement limits. Constructor replacement and broader raw JNI-native admission remain deferred. |
