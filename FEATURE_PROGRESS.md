...
| Constructors, methods, static methods, fields | Done | `JavaClass::{new,new_with,constructor}`, `JavaConstructor`, `JavaMethod`, `JavaField` | Reflection-validated overload/member handles with typed helpers for primitive, object, and array fields; selected instance handles validate receiver class before JNI; `new()` and name-only calls use ranked argument dispatch, while exact overload handles remain available. |
